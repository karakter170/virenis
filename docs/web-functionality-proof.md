...
| New chat, send, stream, cancel/recover, retry, copy, feedback | `src/App.jsx` | chat/run/SSE routes in `server/app.js`, coordinated by `server/chatRunCoordinator.js` | `api.test.js`, `sseLifecycle.test.js`, `recovery.test.js`, `sessionRunAdmission.test.js`, `chatLifecycleUi.test.js` | Marketing and Brainstorming scripts submit through the real composer and wait for the durable GPU run. |
...
| Agent creation and editing | `AgentBuilderDialog` in `src/App.jsx` | agent CRUD routes, `server/agentContract.js`, `server/agentRuntimeClient.js` | `agentContractV4.test.js`, `agentToolConfiguration.test.js`, `workflowAgentConfigurationE2E.test.js`, `api.test.js` | Agent Builder browser proof creates an agent, verifies the persisted JSON and live Runtime manifest, then archives and purges it. |
...
| Human-readable answers and named provenance | formatted answer, source hover card, right-side Answer Details panel | route/output provenance in `server/routeResultNormalizer.js`; presentation normalization in `src/answerPresentation.js` | `answerPresentation.test.js`, `productFeatures.test.js`, `runtimeStreaming.test.js` | Marketing browser proof checks headings, no raw Runtime IDs/schema envelopes, named agent controls, hover summary, focused source panel, and every specialist output. |
| Runtime status, adoption, audit, model/output limits | status/admin panels | `server/agentRuntimeClient.js`, `server/modelSettings.js`, Runtime audit routes | `runtimeClient.test.js`, `runtimeAdoption.test.js`, `runtimeAuditReconciliation.test.js`, `modelSettingsApi.test.js` | Agent Builder and curated live scripts cross the web-to-Runtime boundary and inspect the registered Runtime agent. |
...
12. Python and web terminal validators still demanded a legacy six-item English policy checklist after the semantic model had produced a valid semantic-first plan. Python rejected before execution; the web then rejected a later run after all six agents had completed. Both boundaries now preserve the enum-closed `semantic_model_led` authority and accept `semantic_policy_diagnostics_only`, while missing/tampered authority markers remain fail-closed.
