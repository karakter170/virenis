...
Qwen sees all agents in the active team (maximum 16) and is the semantic selector. A second independent Qwen adjudication reviews every proposal against the current utterance in its original language, recent context, and the same complete team catalog. No keyword, phrase, regex, language list, exact-output match, mention parser, or deterministic cue score may choose, add, remove, or veto an agent. If model JSON is malformed, Qwen receives one schema-repair request; if semantic routing is still unavailable, the base model answers directly instead of guessing a specialist. Runtime independently validates only structural execution facts such as authorized IDs, outputs, permissions, lifecycle, the 16-agent ceiling, and saved dependency closure. Runtime readiness also compares the configured 32,768-token context window with the provider's advertised `max_model_len`, preventing a smaller Qwen deployment from silently degrading selection.

The executor gives literal user constraints a content-bound `query:<digest>` identity and validated route results their own content-bound evidence IDs. Markers apply only to the clause or structured `claim:` field that carries them; they cannot spread across a semicolon or through sibling metadata to bless a new recommendation. One-output and terminal agents return only a validated domain answer, which the executor wraps into the declared artifact. Four-or-more-result fan-in uses a coverage-preserving representation. Within the 32,768-token model window, specialists receive up to 8,192 input tokens and default to a 4,096-token output budget; final synthesis receives up to 16,384 input tokens and defaults to an 8,192-token output budget. Administrator ceilings are 8,192 specialist-output tokens and 12,288 synthesis-output tokens, with context-safe clamping always taking precedence.
